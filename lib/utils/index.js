...
function createPathFromEndpoint(version, endpoint, singularList, params, token) {
    var path = '/' + version;
    ...
    for (var i = 0; i < parts.length - 1; i++) {
        var pluralPart = decamelize(parts[i], '-');
        ...
        // don't pluralize exceptions
        if (singularList.indexOf(parts[i]) === -1) {
            pluralPart = inflector.pluralize(pluralPart);
        }
        path += '/' + pluralPart;
